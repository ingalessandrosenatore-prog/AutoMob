...
//  Oppure:         dart run <repo_root>/automob_backoffice_mech/tool/check_architecture.dart
//                 (dal root del mono-repo)
//
...
// Risolve la root del progetto rispetto al percorso di questo script,
// così funziona indipendentemente dal CWD.
String _projectRoot() {
  final script = Platform.script.toFilePath();
  // script = 'file:///C:/.../automob_backoffice_mech/tool/check_architecture.dart'
  final path = script.replaceAll('file://', '').replaceAll('/', r'\');
  final toolDir = Directory(path).parent.path;
...
String get _baselineFile => '${_projectRoot()}/tool/arch_baseline.txt';
...
void main() {
  ...
  for (final entity in libDir.listSync(recursive: true)) {
    ...
    final rel = _libRelative(entity.path); // es: features/auth/data/...
  ...
  if (baselinate > 0) {
    print(
      '($baselinate violazioni a BASELINE — debito tecnico tracciato in $_baselineFile)',
    );
  }
...
// Ritorna la path relativa a lib/ (posix), es: features/auth/data/foo.dart
...
void _checkImport(
  ...
  if (target.startsWith('package:')) {
    ...
    if (pkg == 'automob_backoffice_mech') {
      final internal = target.substring(
        'package:automob_backoffice_mech/'.length,
      );
...
// restituendo una path relativa a lib/ (es: features/auth/domain/...).
