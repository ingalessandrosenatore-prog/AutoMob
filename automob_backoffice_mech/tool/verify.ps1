# =====================================================================
#  verify.ps1  —  CANCELLO DI QUALITA' (app meccanico)
# ---------------------------------------------------------------------
#  Esegue, in ordine, tutti i controlli. Se UNO fallisce -> exit 1,
#  cosi' chi lo usa (es. ship.ps1) NON committa.
#
#  Uso:   ./tool/verify.ps1
#         (da qualunque cartella: lo script usa la root dell'app meccanica)
# =====================================================================

$projectRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$verifyRepoRoot = (Resolve-Path (Join-Path $projectRoot "..")).Path
...
try {
Write-Host ""
Write-Host "==> 1/6  File vietati e configurazione locale" -ForegroundColor Cyan
$trackedFiles = @(
...
Write-Host ""
Write-Host "==> 2/6  Formattazione Dart dei file modificati" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "==> 3/6  Architettura" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "==> 4/6  Copertura strutturale test" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "==> 5/6  flutter analyze --fatal-infos" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "==> 6/6  flutter test" -ForegroundColor Cyan
flutter test
if ($LASTEXITCODE -ne 0) {
...
Write-Host ""
Write-Host "OK  Tutto verde: file + format + architettura + test coverage + analyze + test Flutter." -ForegroundColor Green
exit 0
} finally {
    Pop-Location
}
